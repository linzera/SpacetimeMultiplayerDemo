...
namespace SpacetimeDB.Types
{
    ...
    public sealed partial class RemoteTables : RemoteTablesBase
    {
        ...
        {
            AddTable(ActiveTrade = new(conn));
            AddTable(Animation = new(conn));
            AddTable(CheckChunksForAllPlayersTimer = new(conn));
            AddTable(Chunk = new(conn));
            AddTable(ChunkData = new(conn));
            AddTable(Config = new(conn));
            AddTable(DespawnNpcsTimer = new(conn));
            AddTable(Inventory = new(conn));
            AddTable(MoveNpcsTimer = new(conn));
            AddTable(Npc = new(conn));
            AddTable(Player = new(conn));
            AddTable(PlayerChatMessage = new(conn));
            AddTable(PlayerLogin = new(conn));
            AddTable(Resource = new(conn));
            AddTable(ServerGlobals = new(conn));
            AddTable(SpawnNpcsTimer = new(conn));
            AddTable(Trade = new(conn));
            AddTable(Transform = new(conn));
        }
    }
    ...
    public sealed class DbConnection : DbConnectionBase<DbConnection, RemoteTables, Reducer>
    {
        ...
        protected override Reducer ToReducer(TransactionUpdate update)
        {
            ...
            return update.ReducerCall.ReducerName switch
            {
                ...
                "add_to_trade" => BSATNHelpers.Decode<Reducer.AddToTrade>(encodedArgs),
                "check_chunks_for_all_players" => BSATNHelpers.Decode<Reducer.CheckChunksForAllPlayers>(encodedArgs),
                "create_new_player" => BSATNHelpers.Decode<Reducer.CreateNewPlayer>(encodedArgs),
                "despawn_npcs" => BSATNHelpers.Decode<Reducer.DespawnNpcs>(encodedArgs),
                "dump_inventory" => BSATNHelpers.Decode<Reducer.DumpInventory>(encodedArgs),
                ...
                "initiate_trade_session" => BSATNHelpers.Decode<Reducer.InitiateTradeSession>(encodedArgs),
                "move_npc" => BSATNHelpers.Decode<Reducer.MoveNpc>(encodedArgs),
                "move_npcs" => BSATNHelpers.Decode<Reducer.MoveNpcs>(encodedArgs),
                "move_or_swap_inventory_slot" => BSATNHelpers.Decode<Reducer.MoveOrSwapInventorySlot>(encodedArgs),
                "move_player" => BSATNHelpers.Decode<Reducer.MovePlayer>(encodedArgs),
                "player_chat" => BSATNHelpers.Decode<Reducer.PlayerChat>(encodedArgs),
                "player_update_login_state" => BSATNHelpers.Decode<Reducer.PlayerUpdateLoginState>(encodedArgs),
                "refuse_trade" => BSATNHelpers.Decode<Reducer.RefuseTrade>(encodedArgs),
                "remove_from_trade" => BSATNHelpers.Decode<Reducer.RemoveFromTrade>(encodedArgs),
                "spawn_npcs" => BSATNHelpers.Decode<Reducer.SpawnNpcs>(encodedArgs),
                "toggle_accept_trade" => BSATNHelpers.Decode<Reducer.ToggleAcceptTrade>(encodedArgs),
                "update_animation" => BSATNHelpers.Decode<Reducer.UpdateAnimation>(encodedArgs),
                "update_npc_animation" => BSATNHelpers.Decode<Reducer.UpdateNpcAnimation>(encodedArgs),
        ...
        protected override bool Dispatch(IReducerEventContext context, Reducer reducer)
        {
            ...
            return reducer switch
            {
                ...
                Reducer.AddToTrade args => Reducers.InvokeAddToTrade(eventContext, args),
                Reducer.CheckChunksForAllPlayers args => Reducers.InvokeCheckChunksForAllPlayers(eventContext, args),
                Reducer.CreateNewPlayer args => Reducers.InvokeCreateNewPlayer(eventContext, args),
                Reducer.DespawnNpcs args => Reducers.InvokeDespawnNpcs(eventContext, args),
                Reducer.DumpInventory args => Reducers.InvokeDumpInventory(eventContext, args),
                ...
                Reducer.InitiateTradeSession args => Reducers.InvokeInitiateTradeSession(eventContext, args),
                Reducer.MoveNpc args => Reducers.InvokeMoveNpc(eventContext, args),
                Reducer.MoveNpcs args => Reducers.InvokeMoveNpcs(eventContext, args),
                Reducer.MoveOrSwapInventorySlot args => Reducers.InvokeMoveOrSwapInventorySlot(eventContext, args),
                Reducer.MovePlayer args => Reducers.InvokeMovePlayer(eventContext, args),
                Reducer.PlayerChat args => Reducers.InvokePlayerChat(eventContext, args),
                Reducer.PlayerUpdateLoginState args => Reducers.InvokePlayerUpdateLoginState(eventContext, args),
                Reducer.RefuseTrade args => Reducers.InvokeRefuseTrade(eventContext, args),
                Reducer.RemoveFromTrade args => Reducers.InvokeRemoveFromTrade(eventContext, args),
                Reducer.SpawnNpcs args => Reducers.InvokeSpawnNpcs(eventContext, args),
                Reducer.ToggleAcceptTrade args => Reducers.InvokeToggleAcceptTrade(eventContext, args),
                Reducer.UpdateAnimation args => Reducers.InvokeUpdateAnimation(eventContext, args),
                Reducer.UpdateNpcAnimation args => Reducers.InvokeUpdateNpcAnimation(eventContext, args),
